"""Scraper for events from bielefeld.de (Stadt Bielefeld)."""
...
class StadtwerkeBielefeldScraper(BaseScraper):
    """Scrapes events from the official city of Bielefeld website."""
    ...
    def scrape(self) -> list[Event]:
        ...
        try:
            html = self._get_page(
                f"{self.base_url}/de/buerger/stadtleben/veranstaltungen/"
            )
            soup = BeautifulSoup(html, "lxml")

            for card in soup.select(
                ".veranstaltung, .event-item, article, .termin-item"
            ):
    ...
    def _parse_card(self, card) -> Event | None:
        title_el = card.select_one("h2, h3, .titel, .title, a")
        if not title_el:
            return None
        ...
        date_el = card.select_one("time, .datum, .date")
        date_start = self._parse_date(date_el)
        if not date_start:
            return None

        desc_el = card.select_one("p, .beschreibung, .text")
        description = desc_el.get_text(strip=True) if desc_el else ""

        loc_el = card.select_one(".ort, .location, .venue")
        ...
        cat_el = card.select_one(".kategorie, .category")
